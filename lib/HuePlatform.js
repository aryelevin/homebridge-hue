...
function HuePlatform (log, configJson, homebridge) {
  ...
  try {
    if (!fsLib.existsSync(savedStateFilesPath)) {
      this.log.info('States folder is missing, going to create...')
    ...
    this.log.info("The platform state file was loaded!")
  } catch (err) {
    ...
    if (this.config.brightnessAdjustment) {
      ...
      this.state.al_adjustment_factor = this.config.brightnessAdjustment = 1.0
    }
...
HuePlatform.prototype.writeStateFile = function() {
  let that = this
  fsLib.writeFile(savedStateFilesPath + "last_platform_state", JSON.stringify(this.state), 'utf8', function(err) {
    if (err) {
      that.log.info(err)
    } else {
