...
HuePlatform.prototype.accessories = async function (callback) {
  ...
  // Added by me: Arye Levin
  this.platformAccessory = new {}
  this.platformAccessory.getServices = function () {
    return this.platformAccessory.serviceList
  }
  this.platformAccessory.service = new homebridgeLib.MyHomeKitTypes(homebridge).Services.HueBridge('Hue Platform Service')//homebridge.hap.Service
  ...
  this.platformAccessory.service.addOptionalCharacteristic(homebridge.hap.Characteristic.Brightness)
  this.platformAccessory.service.getCharacteristic(homebridge.hap.Characteristic.Brightness)
    .updateValue(100)
    .setProps({ minValue: 10 })
    .on('set', this.setBrightness.bind(this))


  this.remotesAndServerEnabledStateService = new homebridge.hap.Service.Switch('Hue Control Enabled')
  this.remotesAndServerEnabledStateService.getCharacteristic(homebridge.hap.Characteristic.On)
