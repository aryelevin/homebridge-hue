  ...
  // End of Added by me: Arye Levin
    ...
    if (this.config.ct) {
      ...
      this.service.getCharacteristic(my.Characteristics.ActiveTransitionCount)
        .updateValue(0)
      // Added by me: Arye Levin
      this.service.addOptionalCharacteristic(my.Characteristics.Volume)
      this.service.getCharacteristic(my.Characteristics.Volume)
        .updateValue(100)
...
HueLight.prototype.checkOn = function (on) {
  ...
  if (this.hk.on !== hkOn
    ...
    // Added by me: Arye Levin
    if (this.hk.on === undefined || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer' || this.manufacturer === 'Danalock') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since they're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
      this.hk.on = hkOn
      ...
      // this.hk.on = !!originalOnState
      this.setOn(/*originalOnState*/this.hk.on, function(){
...
HueLight.prototype.checkAllOn = function (allOn) {
  ...
  if (this.hk.on !== hkOn) {
    ...
    // Added by me: Arye Levin
    if (this.hk.on === undefined || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since the're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
      this.hk.on = hkOn
      this.service.getCharacteristic(Characteristic.On)
        .updateValue(this.hk.on)
    } else {
