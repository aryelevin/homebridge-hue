  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      if (pathComponents[4] === 'switch') {
        ...
        if (anyOn !== this.bridge.fullState.lights[pathComponents[3]].state.on) {
          this.bridge.fullState.lights[pathComponents[3]].state.on = anyOn // To make sure to avoid its socket message with the attribute report...
