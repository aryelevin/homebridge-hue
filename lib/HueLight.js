...
HueLight.prototype.updatePanels = function(parameter, content, hkValueToCheck) {
  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      let panelResourceItem = panelsToUpdate[i]
      let pathComponents = panelResourceItem.split( '/' )
      if (pathComponents[3].startsWith('light') || pathComponents[3].startsWith('curtain')) {
        let lightsControlledWithPanelDevice = this.bridge.panelsToResources[panelResourceItem]
        ...
        if (shouldUpdatePanelState) {
          let panelResourcePath = '/' + pathComponents[1] + '/' + pathComponents[2] + '/state'
          this.bridge.put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + (pathComponents[3].startsWith('light') ? '6c69676874732f' : '6375727461696e') + parseInt(pathComponents[3].charAt(pathComponents[3].length-1)).toString(16).padStart(2, '0') + parameter + content}).then((obj) => {
