...
const knownLights = {
  ...
  GLEDOPTO: {
    ...
    models: {
      'GL-C-008': { noWaitUpdate: true , maxCt : 370 , originalMaxCT : 495 },
      GLEDOPTO: { // Issue #244
        fix: function () {
          if (
            this.subtype === '0a' &&
...
HueLight.prototype.checkCt = function (ct) {
  ...
  if (this.config.originalMaxCT) {
    hkCt = Math.round(ct - ((this.config.originalMaxCT - this.config.maxCt) * ((ct - this.config.minCt) / (this.config.originalMaxCT - this.config.minCt))))
  }
  // End of Added by me: Arye Levin
  if (this.hk.ct !== hkCt) {
...
HueLight.prototype.setCt = function (ct, callback) {
  ...
  if (this.config.originalMaxCT) {
    const calculatedCTForLight = Math.round(newCt + ((this.config.originalMaxCT - this.config.maxCt) * ((newCt - this.config.minCt) / (this.config.maxCt - this.config.minCt))))
