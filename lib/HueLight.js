...
function HueLight (accessory, id, obj, type = 'light') {
  ...
  // Added by me: Arye Levin
  } else if (this.config.doorlock) { // Lock
    this.service = new Service.LockMechanism(this.name, this.subtype)
    this.service.getCharacteristic(Characteristic.LockCurrentState)
    this.service.getCharacteristic(Characteristic.LockTargetState)
...
HueLight.prototype.putState = function () {
  ...
  if (!this.config.outlet && !this.config.switch && !this.config.doorlock && newOn && this.hk.bri) {
...
HueLight.prototype.checkOn = function (on) {
  ...
  if (this.config.wallSwitch && this.obj.state.reachable !== true) {
    ...
    hkOn = /*Added by me: Arye Levin*/(this.config.doorlock && this.service.getCharacteristic(Characteristic.LockCurrentState).value != this.service.getCharacteristic(Characteristic.LockTargetState).value) ? this.hk.on : /*End of Added by me: Arye Levin*/this.obj.state.on
  }
  if (this.hk.on !== hkOn/*Added by me: Arye Levin*/ || (this.config.doorlock && this.service.getCharacteristic(Characteristic.LockCurrentState).value != this.service.getCharacteristic(Characteristic.LockTargetState).value)/*End of Added by me: Arye Levin*/) {
    if (this.hk.on !== undefined) {
      // Added by me: Arye Levin
      if (this.config.doorlock) {
        if (hkOn === this.obj.state.on) {
          this.log.info(
            '%s: set homekit locked from %s to %s', this.name,
            !this.obj.state.on, hkOn
          )
        } else {
          this.log.info(
    ...
    // Added by me: Arye Levin
    if (this.hk.on === undefined || this.config.doorlock || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since they're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
    // End of Added by me: Arye Levin
    this.hk.on = hkOn
    // Added by me: Arye Levin
      if (this.config.doorlock) {
        this.service.getCharacteristic(Characteristic.LockCurrentState)
          .updateValue(this.obj.state.on ? Characteristic.LockCurrentState.SECURED : Characteristic.LockCurrentState.UNSECURED)
        this.service.getCharacteristic(Characteristic.LockTargetState)
          .updateValue(this.hk.on === true ? Characteristic.LockTargetState.SECURED : Characteristic.LockTargetState.UNSECURED)
      } else {
    // End of Added by me: Arye Levin
    this.service.getCharacteristic(Characteristic.On)
...
HueLight.prototype.setOn = function (on, callback) {
  ...
  this.put(request).then(() => {
    ...
    // Added by me: Arye Levin
      ...
      // Do nothing, we have to wait to get the state from the bridge to confirm execution.
    // End of Added by me: Arye Levin
    } else {
