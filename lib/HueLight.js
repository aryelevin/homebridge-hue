...
HueLight.prototype.putState = function () {
  ...
  if (!this.config.outlet && !this.config.switch && newOn) {
...
HueLight.prototype.checkOn = function (on) {this.log.info( '%s: checkOn', this.name )
  if (this.config.windowCovering) {
  ...
  if (this.hk.on !== hkOn
    ...
    if (this.hk.on === undefined || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer' || this.manufacturer === 'Danalock') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since they're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
      ...
      if (this.config.doorlock) {
        this.service.getCharacteristic(Characteristic.LockCurrentState)
          .updateValue(this.obj.state.on === 1 ? Characteristic.LockCurrentState.SECURED : Characteristic.LockCurrentState.UNSECURED)
        this.service.getCharacteristic(Characteristic.LockTargetState)
...
HueLight.prototype.setOn = function (on, callback) {
  ...
  this.put(request).then(() => {
    ...
    // Added by me: Arye Levin
      ...
      // Do not set the state yet, wait for the attibute reporting to do it.
    // End of Added by me: Arye Levin
    } else {
