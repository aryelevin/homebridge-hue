...
HueLight.prototype.setConfig = function () {
  ...
  if (this.config.ct) {
    ...
    // Added by me: Arye Levin
    let resoucreConfig = this.bridge.platform.configJson['resourcesCCTRanges'][this.resource]
    if (resoucreConfig && resoucreConfig.maxHKCt) {
...
HueLight.prototype.putState = function () {
  if (this.hk.mute !== undefined || (this.config.doorLock && this.service.getCharacteristic(Characteristic.LockCurrentState).value == this.service.getCharacteristic(Characteristic.LockTargetState).value)/*this.subtype === 23*/) {
  ...
  if (this.config.ct && this.obj.state.colormode === 'ct' && newOn && this.hk.ct) {
    ...
    if (this.config.originalMaxCT && this.config.actualMaxCt) {
      newCt = Math.min(newCt, this.config.actualMaxCt)
      const calculatedCTForLight = Math.round(newCt + ((this.config.originalMaxCT - this.config.actualMaxCt) * ((newCt - this.config.minCt) / (this.config.actualMaxCt - this.config.minCt))))
...
HueLight.prototype.checkCt = function (ct) {
  ...
  if (this.config.originalMaxCT && this.config.actualMaxCt) {
    hkCt = Math.min(hkCt, this.config.actualMaxCt)
    hkCt = Math.round(ct - ((this.config.originalMaxCT - this.config.actualMaxCt) * ((ct - this.config.minCt) / (this.config.originalMaxCT - this.config.minCt))))
  }
  // End of Added by me: Arye Levin
  if (this.hk.ct !== hkCt) {
...
HueLight.prototype.setOn = function (on, callback) {
  ...
  // Added by me: Arye Levin
  this.updatePanels('04010055', '000000' + (newOn ? 1 : 0).toString(16).padStart(2, '0'), Characteristic.On, this.hk.on)
  let panelsToUpdate = this.bridge.resourcesToPanels[this.resource]
  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      let panelResourcePath = '/' + pathComponents[1] + '/' + pathComponents[2] + '/state'

      if (pathComponents[3] === 'switch') {
        let lightsControlledWithPanelDevice = this.bridge.panelsToResources[panelResourceItem]
        var anyOn = false
        if (newOn) {
          anyOn = true
        } else {
          for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
            ...
            let accessoryToCheck = this.bridge[lightResourcePath[1]][lightResourcePath[2]]
            if (accessoryToCheck && accessoryToCheck.service.testCharacteristic(Characteristic.On)) {
              if (accessoryToCheck !== this && accessoryToCheck.service.getCharacteristic(Characteristic.On).value) { // this.obj.state.on
                console.log('++++++++ light on: ' + lightResourcePath + ', this light id: ' + this.resource)
                anyOn = true
                break
              }
            }
        ...
        if (anyOn !== this.bridge.fullState.lights[pathComponents[2]].state.on) {
          this.bridge.fullState.lights[pathComponents[2]].state.on = anyOn // To make sure to avoid its socket message with the attribute report...
          this.bridge.put(panelResourcePath, {on: anyOn}).then((obj) => {
...
HueLight.prototype.setBri = function (bri, callback) {
  ...
  // Added by me: Arye Levin
  this.updatePanels('0e010055', '000000' + bri.toString(16).padStart(2, '0'), Characteristic.Brightness, this.hk.bri)
  this.writeHKStateFile()
  // End of Added by me: Arye Levin
}
...
HueLight.prototype.setCt = function (ct, callback) {
  ...
  // Added by me: Arye Levin
  this.updatePanels('0e020055', '0000' + ct.toString(16).padStart(4, '0'), Characteristic.ColorTemperature, this.hk.ct)
  this.writeHKStateFile()
  // End of Added by me: Arye Levin
}
...
HueLight.prototype.setHue = function (hue, callback) {
  ...
  // Added by me: Arye Levin
  const newXy = hsvToXy(this.hk.hue, this.hk.sat, this.config.gamut)
  this.updatePanels('0e080055', Math.round(newXy[0] * 65535).toString(16).padStart(4, '0') + Math.round(newXy[1] * 65535).toString(16).padStart(4, '0'), Characteristic.Hue, this.hk.hue)
  this.writeHKStateFile()
  // End of Added by me: Arye Levin
}
...
HueLight.prototype.setSat = function (sat, callback) {
  ...
  // Added by me: Arye Levin
  const newXy = hsvToXy(this.hk.hue, this.hk.sat, this.config.gamut)
  this.updatePanels('0e080055', Math.round(newXy[0] * 65535).toString(16).padStart(4, '0') + Math.round(newXy[1] * 65535).toString(16).padStart(4, '0'), Characteristic.Saturation, this.hk.sat)
  this.writeHKStateFile()
  // End of Added by me: Arye Levin
}
...
HueLight.prototype.updatePanels = function(parameter, content, characteristicToCheck, currentCharacteristicValue) {
  let panelsToUpdate = this.bridge.resourcesToPanels[this.resource]
  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
        let lightResourcePath = lightsControlledWithPanelDevice[ii].split('/')
        let accessoryToCheck = this.bridge[lightResourcePath[1]][lightResourcePath[2]]
        if (accessoryToCheck && accessoryToCheck.service.testCharacteristic(characteristicToCheck)) {
          if (accessoryToCheck !== this && accessoryToCheck.service.getCharacteristic(characteristicToCheck).value != currentCharacteristicValue) { // this.obj.state.on
            console.log('light: ' + lightResourcePath + ', value: ' + accessoryToCheck.service.getCharacteristic(characteristicToCheck).value + ', new Value: ' + currentCharacteristicValue)
      ...
      if (pathComponents[3].startsWith('light') && shouldUpdatePanelState) {
        this.bridge.put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d210541106c69676874732f' + parseInt(pathComponents[3].charAt(pathComponents[3].length-1)).toString(16).padStart(2, '0') + parameter + content}).then((obj) => {
