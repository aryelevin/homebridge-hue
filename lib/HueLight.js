  ...
  setTimeout(function() {
    ...
    if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
      for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
        ...
        if (pathComponents[4] === 'switch' && this.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on != on) {
          ...
          if (newOn) {
            ...
            for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
              ...
              if (accessoryToCheck) {
                if (accessoryToCheck !== this && accessoryToCheck.hk.on) {
                  anyOn = true
                  break
                }
              }
          ...
          if (anyOn !== this.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on) {
            this.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on = anyOn // To make sure to avoid its socket message with the attribute report...
            let panelResourcePath = '/' + pathComponents[2] + '/' + pathComponents[3] + '/state'
            this.bridge.platform.bridgeMap[pathComponents[1]].put(panelResourcePath, {on: anyOn}).then((obj) => {
