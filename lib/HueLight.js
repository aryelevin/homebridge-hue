...
const knownLights = {
  ...
  Sunricher: {
    models: {
      'RGB-CCT': { maxCt : 500 , originalMaxCT : 450 , actualMaxCt: 370} // maxCt: the range to use in HK (for the Adaptive lighting to work properly [and in sync with other lights in the house]). originalMaxCT: the driver logical max CT. actualMaxCt: the actual light LED chip max CT.
    }
  },
  // End of Added by me: Arye Levin
  GLEDOPTO: {
    ...
    models: {
      'GL-C-008': { noWaitUpdate: true , maxCt : 500 , originalMaxCT : 495 , actualMaxCt: 370}, // Added by me: Arye Levin
      GLEDOPTO: { // Issue #244
        fix: function () {
          if (
            this.subtype === '0a' &&
        ...
        }, maxCt : 500 , originalMaxCT : 500 , actualMaxCt: 400 // Added by me: Arye Levin
      }
    }
  },
  icasa: {
...
function HueLight (accessory, id, obj, type = 'light') {
  ...
  if (this.config.windowCovering) {
    ...
    if (this.config.ct) {
      ...
      this.service.getCharacteristic(Characteristic.Volume)
        ...
        .on('set', this.setAdaptiveLightingFactor.bind(this))
      this.hk.alfactor = 1;
      // End of Added by me: Arye Levin
    }
  ...
  // Added by me: Arye Levin
  try {
    let data = fs.readFileSync(savedStateFilesPath + "last_device_state_" + this.serialNumber, 'utf8')
    let parsedJSON = JSON.parse(data)
    if (!this.config.doorLock && parsedJSON['on']) {
      this.service.getCharacteristic(Characteristic.On).setValue(parsedJSON['on'])
    }
    if (this.config.bri) {
      this.service.getCharacteristic(Characteristic.Brightness).setValue(parsedJSON['bri'])
    }
    if (this.config.ct) {
      if (parsedJSON['al']) {
        this.hk.al = parsedJSON['al']
      } else {
        this.service.getCharacteristic(this.colorTemperatureCharacteristic).setValue(parsedJSON['ct'])
        console.log('No al key found for a ct capable light, the data of the state file is: ' + data)
      }
      if (parsedJSON['alfactor']) {
        let alFactor = parsedJSON['alfactor']
        if (alFactor >= 0.1 && alFactor <= 1) {
          this.hk.alfactor = alFactor
          this.service.getCharacteristic(Characteristic.Volume).updateValue(this.hk.alfactor * 100)
        }
      }
    }
		this.log.info(this.name + ': The device\'s file was loaded!')
	} catch (err) {
		// Here you get the error when the file was not found,
		// but you also get any other error
		if (err.code === 'ENOENT') {
			this.log.info(this.name + ': File not found!')
		} else {
			this.log.info(this.name + ': ' + err)
			// throw err
		}
  }
  // End of Added by me: Arye Levin
...
HueLight.prototype.setConfig = function () {
  ...
  if (this.config.ct) {
    if (model.minCt != null) {
      ...
      // Added by me: Arye Levin
      if (model.originalMinCT != null) {
        this.config.originalMinCT = model.originalMinCT
      }
      if (model.actualMinCt != null) {
        this.config.actualMinCt = model.actualMinCt
      }
      // End of Added by me: Arye Levin
      ...
      // Added by me: Arye Levin
      if (manufacturer.originalMinCT != null) {
        this.config.originalMinCT = manufacturer.originalMinCT
      }
      if (manufacturer.actualMinCt != null) {
        this.config.actualMinCt = manufacturer.actualMinCt
      }
      // End of Added by me: Arye Levin
    ...
    if (model.maxCt != null) {
      ...
      // Added by me: Arye Levin
      if (model.originalMaxCT != null) {
        this.config.originalMaxCT = model.originalMaxCT
      }
      if (model.actualMaxCt != null) {
        this.config.actualMaxCt = model.actualMaxCt
      }
      // End of Added by me: Arye Levin
      ...
      // Added by me: Arye Levin
      if (manufacturer.originalMaxCT != null) {
        this.config.originalMaxCT = manufacturer.originalMaxCT
      }
      if (manufacturer.actualMaxCt != null) {
        this.config.actualMaxCt = manufacturer.actualMaxCt
      }
      // End of Added by me: Arye Levin
    } else if (
      this.obj.ctmax != null &&
    ...
    // Added by me: Arye Levin
    let resoucreConfig = this.bridge.platform.configJson['resourcesCCTRanges']['/' + this.bridge.bridge.bridgeid + this.resource]
    if (resoucreConfig) {
      if (resoucreConfig.maxHKCt) {
        this.config.maxCt = resoucreConfig.maxHKCt
        if (resoucreConfig.deviceLogicMaxCT) {
          this.config.originalMaxCT = resoucreConfig.deviceLogicMaxCT
        }
        if (resoucreConfig.actualLightMaxCt) {
          this.config.actualMaxCt = resoucreConfig.actualLightMaxCt
        }
      }
      if (resoucreConfig.minHKCt) {
        this.config.minCt = resoucreConfig.minHKCt
        if (resoucreConfig.deviceLogicMinCT) {
          this.config.originalMinCT = resoucreConfig.deviceLogicMinCT
        }
        if (resoucreConfig.actualLightMinCt) {
          this.config.actualMinCt = resoucreConfig.actualLightMinCt
        }
      }
    }
    // End of Added by me: Arye Levin
...
// Added by me: Arye Levin
HueLight.prototype.putState = function () {
  if (this.hk.mute !== undefined || this.config.windowCovering || (this.config.doorLock && this.service.getCharacteristic(Characteristic.LockCurrentState).value == this.service.getCharacteristic(Characteristic.LockTargetState).value)/*this.subtype === 23*/) {
    // this.log.info('putState: %s is most probably a warning device such as a smoke sensor, skipping...', this.resource);
    return
  }

  // this.log.info('putState: %s', this.resource);
  const newOn = this.config.doorLock ? this.hk.lockTargetState === Characteristic.LockTargetState.SECURED : this.hk.on
  var request = { on: newOn ? true : false }
  if (!this.config.outlet && !this.config.switch && newOn && this.hk.bri) {
    const newBri = Math.round(this.hk.bri * 254.0 / 100.0)
    request['bri'] = newBri
  }
  if (this.config.ct && this.obj.state.colormode === 'ct' && newOn && this.hk.ct) {
    let newCt = this.hk.ct
    if (this.config.originalMaxCT && this.config.actualMaxCt && this.config.originalMinCT && this.config.actualMinCt) {
      newCt = Math.max(this.config.actualMinCt, Math.min(this.config.actualMaxCt, newCt))
      const calculatedCTForLight = Math.round((((newCt - this.config.actualMinCt) * (this.config.originalMaxCT - this.config.originalMinCT)) / (this.config.actualMaxCt - this.config.actualMinCt)) + this.config.originalMinCT)
      newCt = calculatedCTForLight
    }
    request['ct'] = newCt
  }
  ...
  this.put(request).then(() => {
    // this.log.info('putStateCompleted for: ' + this.resource);
  }).catch((err) => {
...
HueLight.prototype.checkOn = function (on) {
  ...
  if (this.config.doorLock) {
    const hkLockState = /*Added by me: Arye Levin*/this.service.getCharacteristic(Characteristic.LockCurrentState).value != this.service.getCharacteristic(Characteristic.LockTargetState).value ? this.hk.lockTargetState : /*End of Added by me: Arye Levin*/on
      ? Characteristic.LockCurrentState.SECURED
      : Characteristic.LockCurrentState.UNSECURED
    if (/*Added by me: Arye Levin*/this.hk.lockTargetState/*this.hk.lockCurrentState*/ !== hkLockState || this.service.getCharacteristic(Characteristic.LockCurrentState).value != this.service.getCharacteristic(Characteristic.LockTargetState).value/*End of Added by me: Arye Levin*/) {
      if (this.hk.lockCurrentState !== undefined) {
        ...
        // Added by me: Arye Levin
        if (!!hkLockState === this.obj.state.on) {
          this.log.info(
            '%s: set homekit locked from %s to %s', this.name,
            !this.obj.state.on, !!hkLockState
          )
        } else {
          this.log.info(
            '%s: set homekit locking from %s to %s', this.name,
            this.obj.state.on, !!hkLockState
          )
        }
        // End of Added by me: Arye Levin
    ...
    // Added by me: Arye Levin
    this.hk.lockTargetState = hkLockState
    this.service.getCharacteristic(Characteristic.LockTargetState)
      .updateValue(this.hk.lockTargetState)
    // this.hk.lockCurrentState = hkLockState
    this.hk.lockCurrentState = on ? Characteristic.LockCurrentState.SECURED : Characteristic.LockCurrentState.UNSECURED
    // End of Added by me: Arye Levin
    this.service.getCharacteristic(Characteristic.LockCurrentState)
  ...
  if (this.hk.on !== hkOn) {
    ...
    // Added by me: Arye Levin
    if (this.hk.on === undefined || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer' || this.manufacturer === 'Idinio' || this.model === 'lumi.switch.n4acn4') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since they're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
    // End of Added by me: Arye Levin
    this.hk.on = hkOn
    this.service.getCharacteristic(Characteristic.On)
    ...
    this.checkAdaptiveLighting()
    // Added by me: Arye Levin
      this.updatePanelsOnState(hkOn)
    } else {
      // This is to make sure the setOn function will go to send the put command to update the light state itself just in case it is out of sync.
      this.setOn(this.hk.on, function(){
...
HueLight.prototype.checkAllOn = function (allOn) {
  ...
  if (this.hk.on !== hkOn) {
    ...
    // Added by me: Arye Levin
    if (this.hk.on === undefined) { // For the initial state received (can be removed maybe as we restore the state of the lights/groups in the startup but maybe for case there is no saved state we can keep it...)...
    // End of Added by me: Arye Levin
    this.hk.on = hkOn
    this.service.getCharacteristic(Characteristic.On)
      .updateValue(this.hk.on)
    // Added by me: Arye Levin
      ...
      var originalOnState = this.hk.on
      this.obj.state.all_on = originalOnState
      // this.hk.on = originalOnState === 0 ? 1 : 0
      this.setOn(originalOnState, function(){
...
HueLight.prototype.checkBri = function (bri) {
  ...
  if (this.hk.bri !== hkBri) {
    ...
    // Added by me: Arye Levin
    if (this.hk.bri === undefined || this.manufacturer === 'ubisys' || this.manufacturer === 'EcoDim B.V' || this.manufacturer === 'ROBB smarrt' || this.manufacturer === 'ORVIBO' || this.manufacturer === 'AduroSmart Eria' || this.manufacturer === 'SmartDimmer' || this.manufacturer === 'Idinio') { // For the initial state from the device and allowing ubisys devices to be controlled locally (since they're remebers their state after power cycle anyway [still need to think about changes happening while power outage])...
    // End of Added by me: Arye Levin
    this.hk.bri = hkBri
    this.service.getCharacteristic(Characteristic.Brightness)
    ...
    this.checkAdaptiveLighting()
    // Added by me: Arye Levin
      this.updatePanels('0e010055', '000000' + hkBri.toString(16).padStart(2, '0'), 'bri')
    } else {
      var originalBriState = this.hk.bri
      this.obj.state.bri = originalBriState
      this.hk.bri = hkBri
      this.setBri(originalBriState, function(){
...
HueLight.prototype.checkCt = function (ct) {
  ...
  let hkCt = Math.max(this.config.minCt, Math.min(this.config.maxCt, ct))
  if (this.config.originalMaxCT && this.config.actualMaxCt && this.config.originalMinCT && this.config.actualMinCt) {
    hkCt = Math.max(this.config.actualMinCt, Math.min(this.config.actualMaxCt, hkCt))
    hkCt = Math.round((((ct - this.config.originalMinCT) * (this.config.actualMaxCt - this.config.actualMinCt)) / (this.config.originalMaxCT - this.config.originalMinCT)) + this.config.actualMinCt)
  }
  // End of Added by me: Arye Levin
  if (this.hk.ct !== hkCt) {
...
HueLight.prototype.checkLift = function (lift) {
  ...
  if (this.hk.currentPosition !== hkPosition) {
    ...
    this.updatePanels('01010055', this.bridge.getAqaraHexFromInt(this.hk.currentPosition).toString(16).padStart(4, '0')/*positionToAqaraHex[this.hk.currentPosition]*/ + '0000', 'targetPosition')
...
HueLight.prototype.setOn = function (on, callback) {
  ...
  // Added by me: Arye Levin
  this.updatePanelsOnState(newOn)
  this.writeHKStateFile()
  // End of Added by me: Arye Levin
}

// Added by me: Arye Levin
HueLight.prototype.updatePanelsOnState = function (newOn) {
  ...
  let that = this
  setTimeout(function() {
    let panelsToUpdate = that.bridge.platform.resourcesToPanels['/' + that.bridge.bridge.bridgeid + that.resource]
    ...
    if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
      for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
        ...
        if (pathComponents[4] === 'switch'/* && that.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on != that.hk.on*/) {
          let lightsControlledWithPanelDevice = that.bridge.platform.panelsToResources[panelResourceItem]
          var anyOn = false
          if (newOn) {
            anyOn = true
          } else {
            for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
              ...
              let accessoryToCheck = that.bridge.platform.bridgeMap[lightResourcePath[1]][lightResourcePath[2]][lightResourcePath[3]]
              if (accessoryToCheck) {
                if (accessoryToCheck !== that && accessoryToCheck.hk.on) {
                  anyOn = true
                  break
                }
              }
          ...
          if (anyOn !== that.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on) {
            that.bridge.platform.bridgeMap[pathComponents[1]].fullState.lights[pathComponents[3]].state.on = anyOn // To make sure to avoid its socket message with the attribute report...
            let panelResourcePath = '/' + pathComponents[2] + '/' + pathComponents[3] + '/state'
            that.bridge.platform.bridgeMap[pathComponents[1]].put(panelResourcePath, {on: anyOn}).then((obj) => {
...
HueLight.prototype.setCt = function (ct, callback) {
  ...
  this.put({ ct: newCt }).then(() => {
    ...
    this.checkXy(ctToXy(this.hk.ct), true)
    // End of Added by me: Arye Levin
    callback()
  }).catch((error) => {
    this.hk.ct = oldCt
...
HueLight.prototype.setPosition = function (position, callback) {
  ...
  this.put({ lift: newLift }).then(() => {
    ...
    // Added by me: Arye Levin
    this.updatePanels('0e020055', '000000' + (positionState === Characteristic.PositionState.INCREASING ? 1 : 0).toString(16).padStart(2, '0'), 'targetPosition')
    // End of Added by me: Arye Levin
  }).catch((error) => {
...
HueLight.prototype.getSupportedTransitionConfiguration = function (callback) {
  try {
    ...
    // Added by me: Arye Levin
    if (this.hk.al && this.hk.al !== 0) {
      const control = this.hk.al
      this.log.info(
        '%s: get homekit transition control to %j - for state restore', this.name,
        this.al.parseControl(control)
      )
      this.setTransitionControl(control, function () {})
    }
    // End of Added by me: Arye Levin
...
HueLight.prototype.getTransitionControl = function (callback) {
  try {
    ...
    this.log.debug(
      '%s: get homekit transition control to %j', this.name, control
    )
    this.log.info(
      '%s: get homekit transition control to %j', this.name,
      this.al.parseControl(control)
    )
...
HueLight.prototype.checkAdaptiveLighting = function () {
  ...
  const ct = this.al.getCt(
    this.hk.bri * (this.bridge.platform.config.brightnessAdjustment/*Added by me: Arye Levin*/ * this.hk.alfactor/*End of Added by me: Arye Levin*/)
  )
  if (ct == null) {
    return
  }
...
// Added by me: Arye Levin
HueLight.prototype.setAdaptiveLightingFactor = function (vol, callback) {
  const oldFactor = Math.round(this.hk.alfactor * 100)
  if (vol === oldFactor) {
    return callback()
  }
  this.log.info(
    '%s: homekit adaptive lighting factor changed from %d%% to %d%%',
    this.name, oldFactor, vol
  )
  this.hk.alfactor = vol / 100
  this.checkAdaptiveLighting()
  this.writeHKStateFile()
  return callback()
}

HueLight.prototype.writeHKStateFile = function() {
  if (this.writingFile === true) {
    this.waitsForNewWriteHKStateFile = true
  } else {
    this._writeHKStateFile()
  }
}

HueLight.prototype._writeHKStateFile = function() {
  let that = this
  let stateToSave = JSON.stringify(this.hk)
  fs.writeFile(savedStateFilesPath + "last_device_state_" + this.serialNumber, stateToSave, 'utf8', function(err) {
    if (err) {
      that.log.info(err)
    } else {
      that.log.info("The device file was saved!")
    }
    that.writingFile = false
    if (that.waitsForNewWriteHKStateFile) {
      that.waitsForNewWriteHKStateFile = false
      that.writeHKStateFile()
    }
  })
}

HueLight.prototype.updatePanels = function(parameter, content, hkValueToCheck) {
  let panelsToUpdate = this.bridge.platform.resourcesToPanels['/' + this.bridge.bridge.bridgeid + this.resource]
  
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      let panelResourceItem = panelsToUpdate[i]
      let pathComponents = panelResourceItem.split( '/' )
      if (pathComponents[4].startsWith('light') || pathComponents[4].startsWith('curtain')) {
        let lightsControlledWithPanelDevice = this.bridge.platform.panelsToResources[panelResourceItem]
        var shouldUpdatePanelState = true
        for (var ii = lightsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
          let lightResourcePath = lightsControlledWithPanelDevice[ii].split('/')
          let accessoryToCheck = this.bridge.platform.bridgeMap[lightResourcePath[1]][lightResourcePath[2]][lightResourcePath[3]]
          if (accessoryToCheck) {
            if (accessoryToCheck !== this && accessoryToCheck.hk[hkValueToCheck] !== undefined && accessoryToCheck.hk[hkValueToCheck] != this.hk[hkValueToCheck]) { // this.obj.state.on
        ...
        if (shouldUpdatePanelState) {
          let panelResourcePath = '/' + pathComponents[2] + '/' + pathComponents[3] + '/state'
          this.bridge.platform.bridgeMap[pathComponents[1]].put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + (pathComponents[4].startsWith('light') ? '6c69676874732f' : '6375727461696e') + parseInt(pathComponents[4].charAt(pathComponents[4].length-1)).toString(16).padStart(2, '0') + parameter + content}).then((obj) => {
