              ...
              if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01) {
                ...
                if (deviceResourceType) {
                  if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                    ...
                    this.put(resource, {aqara_s1_panel_communication: '19aa7113446d210541106c69676874732f' + deviceSerial[7].toString(16).padStart(2, '0') + '0e080055' + Math.round(x * 65535).toString(16).padStart(4, '0') + Math.round(y * 65535).toString(16).padStart(4, '0')}).then((obj) => {
          
                    }).catch((error) => {
                      
                    })
                  } else if (stateParam[0] === 0x08 && stateParam[1] === 0x00 && stateParam[2] === 0x1f && stateParam[3] === 0xa5 && this.actionsConfigData.aqaraS1Panels['/' + r[1] + '/' + r[2]]) { // Names
                    let name = this.actionsConfigData.aqaraS1Panels['/' + r[1] + '/' + r[2]][deviceResourceType === 'lights/' ? '/light_' + deviceSerial[7] : deviceResourceType === 'curtain' ? '/curtain_' + deviceSerial[7] : 'ac'].name
