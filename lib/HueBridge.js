...
HueBridge.prototype.exposeBridge = async function (obj) {
  ...
  // Added by me: Arye Levin
  try {
    let data = fs.readFileSync(savedStateFilesPath + "/last_enabled_state", 'utf8');
...
HueBridge.prototype.setEnabledState = function(on, callback) {
  ...
  fs.writeFile(savedStateFilesPath + "/last_enabled_state", JSON.stringify({'on': that.onState}), 'utf8', function(err) {
      if (err) {
          that.log(err);
      } else {
