      ...
      for (const panel of panels) {
        ...
        for (const panelControl of panelControls) {
          ...
          if (controlData.resources) {
            this.panelsToResources['/' + this.bridge.bridgeid + panel + '/' + panelControl] = controlData.resources
            for (var i = controlData.resources.length - 1; i >= 0; i--) {
              ...
              this.resourcesToPanels[rid].push('/' + this.bridge.bridgeid + panel + '/' + panelControl)
            }
          }
        }
      }
      this.log.info('panelsToResources: ' + JSON.stringify(this.panelsToResources))
      this.log.info('resourcesToPanels: ' + JSON.stringify(this.resourcesToPanels))
    }
  }
}
...
HueBridge.prototype.setAqaraS1PanelsConfiguration = function() {
      ...
      for (const panel of panels) {
        ...
        if (panelLightObject) {
          ...
          if (panelUniqueId[1] === '01') {
            ...
            try {
              let data = fs.readFileSync(savedStateFilesPath + 'panel_configuration_' + panelLightID, 'utf8')
                      ...
                      setTimeout(function() {
                        ...
                        that.put(panel + '/state', {aqara_s1_panel_communication: cmdToSend}).then((obj) => {
                          ...
                          if (succeededCommands === commmands.length) {
                            ...
                            fs.writeFile(savedStateFilesPath + 'panel_configuration_' + panelLightID, JSON.stringify(parsedData), 'utf8', function(err) {
                              if (err) {
                                that.log.info(err)
                              } else {
  ...
  // TODO: update switches data (state if just added its configuration and icon/name)
}
// End of Added by me: Arye Levin
      ...
      try {
        ...
        if (r[1] === 'sensors' && obj !== undefined && obj.buttonevent !== undefined && this.actionsConfigData && this.actionsConfigData[r[2]] && this.actionsConfigData[r[2]].length && this.state.remotesOn) {
          ...
          if (obj !== undefined && obj['aqara_s1_panel_communication'] !== undefined && obj['aqara_s1_panel_communication'] !== this.fullState.lights[r[2]].state.aqara_s1_panel_communication) {
            ...
            if (dataArray[0] === 0xf2 && dataArray[1] === 0xff && dataArray[2] === 0x41) { // Only commands which the device sent will be proccessed, commands without this begining is just what we sent from here, thus not relevant. (the API push it to our socket automatically since it set to the device state object)
              ...
              if (sum === 512) {
                ...
                if (deviceResourceType) {
                  if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01 && this.state.remotesOn) { // State of device is reported.
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      ...
                      let resources = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/ac']
                      ...
                      let resources = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/curtain_' + deviceSerial[7]]
                      for (var i = resources.length - 1; i >= 0; i--) {
                        let resourceItem = resources[i];
                        let pathComponents = resourceItem.split( '/' )
                        let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
                      ...
                      let resources = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/light_' + deviceSerial[7]]
                      for (var i = resources.length - 1; i >= 0; i--) {
                        let resourceItem = resources[i];
                        let pathComponents = resourceItem.split( '/' )
                        let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
                    ...
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      ...
                      let pathComponents = this.panelsToResources[panelDevicePath][0].split('/')
                      let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
                      ...
                      let pathComponents = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/curtain_' + deviceSerial[7]][0].split( '/' )
                      let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
                      ...
                      let pathComponents = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/light_' + deviceSerial[7]][0].split( '/' )
                      let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
            ...
            if (this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/switch'] !== undefined && this.state.remotesOn) {
              let resources = this.panelsToResources['/' + this.bridge.bridgeid + '/' + r[1] + '/' + r[2] + '/switch']
              for (var i = resources.length - 1; i >= 0; i--) {
                let resourceItem = resources[i];
                let pathComponents = resourceItem.split( '/' )
                let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
