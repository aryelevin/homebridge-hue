...
HueBridge.prototype.exposeBridge = async function () {
  ...
  try {
    ...
    if (this.state.bri) {
      ...
      this.service.getCharacteristic(Characteristic.Brightness).setValue(this.platform.config.brightnessAdjustment)
    }
  ...
  ws
    ...
    .on('changed', (resource, obj) => {
      try {
        ...
        //   console.log(data);
        // }
        if (obj.r === 'sensors' && obj.state !== undefined && this.actionsConfigData && this.actionsConfigData[obj.id] && this.actionsConfigData[obj.id].length && this.state.remotesOn) {
