...
function setHomebridge (homebridge, _my, _eve) {
  ...
  my = _my
  // Added by me: Arye Levin
  savedStateFilesPath = homebridge.user.storagePath() + '/homebridge_hue_states/'
  // End of Added by me: Arye Levin
}
...
HueBridge.prototype.exposeBridge = async function (obj) {
  ...
  // Added by me: Arye Levin
  try {
    var dirname = path.dirname(savedStateFilesPath)
    this.log('Checking states folder path: ' + dirname)
    if (!fs.existsSync(dirname)) {
      this.log('States folder %s is missing, going to create...', dirname)
