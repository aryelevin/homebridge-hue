...
HueBridge.prototype.exposeBridge = async function () {
  ...
  // Added by me: Arye Levin
  try {
    let data = fs.readFileSync(__dirname + "/last_enabled_state", 'utf8');
...
HueBridge.prototype.setEnabledState = function(on, callback) {
  ...
  fs.writeFile(__dirname + "/last_enabled_state", JSON.stringify({'on': that.onState}), 'utf8', function(err) {
      if (err) {
          that.log(err);
      } else {
  ...
  ws
    ...
    .on('changed', (resource, obj) => {
      try {
        ...
        if (obj.r === 'sensors' && obj.state !== undefined && this.actionsConfigData[obj.id] && this.actionsConfigData[obj.id].length && this.onState) {
          var actionsConfig = this.actionsConfigData[obj.id];
          var sensorTypeInt = actionsConfig[0];
          this.log.info('sensor: %s, action: %s, config: %s', obj.id, JSON.stringify(obj), JSON.stringify(actionsConfig));
          // Set the defaults...
          // if (!a.lastActions) {
          //   a.lastActions = {
          //     1002: 0,
