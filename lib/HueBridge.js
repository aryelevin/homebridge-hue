...
HueBridge.prototype.setAqaraS1PanelsConfiguration = function() {
  ...
  if (this.actionsConfigData && Array.isArray(this.actionsConfigData) === false) {
    ...
    if (aqaraS1Panels) {
      ...
      for (const panel of panels) {
        let panelData = aqaraS1Panels[panel]
        let panelLightObject = this.fullState.lights[panel.split('/')[2]]
        if (panelLightObject) {
                    ...
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      ...
                      for (var i = resources.length - 1; i >= 0; i--) {
                        ...
                        if (accessoryToControl) {
                          ...
                          if (onOff) {
                            accessoryToControl.service.getCharacteristic(Characteristic.TargetHeaterCoolerState).setValue(mode === 0 ? Characteristic.TargetHeaterCoolerState.COOL : mode === 1 ? Characteristic.TargetHeaterCoolerState.HEAT : Characteristic.TargetHeaterCoolerState.AUTO)
                            accessoryToControl.service.getCharacteristic(Characteristic.RotationSpeed).setValue(fan === 1 ? 25 : fan === 2 ? 50 : fan === 3 ? 75 : 100)
                            if (mode === 0 || mode === 1) {
                              accessoryToControl.service.getCharacteristic(mode === 0 ? Characteristic.CoolingThresholdTemperature : Characteristic.HeatingThresholdTemperature).setValue(setTemperature)
                            }
                          }
                        }
                      }
