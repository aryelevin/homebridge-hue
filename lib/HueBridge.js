      ...
      for (const panel of panels) {
        ...
        if (panelLightObject) {
          ...
          if (panelUniqueId[1] === '01') {
            ...
            try {
              let data = fs.readFileSync(savedStateFilesPath + 'panelConfiguration_' + panelLightID, 'utf8')
              let parsedData = JSON.parse(data)
            } catch (err) {
              // Here you get the error when the file was not found,
              // but you also get any other error
                      ...
                      setTimeout(function() {
                        ...
                        that.put(panel + '/state', {aqara_s1_panel_communication: cmdToSend}).then((obj) => {
                          ...
                          if (succeededCommands === commandsToExecute.length) {
                            parsedData[deviceSerialIndex] = commandsToExecute
                            fs.writeFile(savedStateFilesPath + 'panelConfiguration_' + panelLightID, JSON.stringify(parsedData), 'utf8', function(err) {
                              if (err) {
                                that.log.info(err)
                              } else {
