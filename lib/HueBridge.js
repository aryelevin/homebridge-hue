    ...
    if (aqaraS1Panels) {
      ...
      for (const panel of panels) {
        ...
        for (var i = devicesSerial.length - 1; i >= 0; i--) {
          var deviceSerial = devicesSerial[i];
          // TODO: now set all as Online, but later make sure to mark offline devices if they're offline...
          this.put(panel + '/state', {aqara_s1_panel_communication: '19aa7113446d21054110' + deviceSerial + '080007fd' + '0000000' + (panelData[devicesControl[i]] ? '1' : '0')}).then((obj) => {
                ...
                if (deviceResourceType) {
                  if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01) { // State of device is reported.
                    ...
                    if (dataArray[13] === 0x01) { // A device is missing...
                      if (deviceResourceType === 'lights/') {
                        let deviceName = 'light_' + deviceSerial[7]
                        console.log(deviceName)
                        let deviceConfig = this.actionsConfigData.aqara_S1_panels['/' + r[1] + '/' + r[2]][deviceName]
                        console.log(deviceConfig)
                        if (deviceConfig && slotsRanges[deviceName]) {
                          ...
                          // TODO: Check that a light config haven't changed in a new config (Need to check how this is possible to be done... Maybe try to set CT/Color and see what is the response...)
                          let panelSerial = this.fullState.lights[r[2]].uniqueid.split('-')[0].replace(/:/g, '')
                          let lightSerial = '6c69676874732f' + deviceSerial[7].toString(16).padStart(2, '0')
                          // On/Off, general type...
                          let cmd1 = '40aa7138468c0201d8024140604f7448' + slotsRanges[deviceName][0].toString(16).padStart(2, '0') + panelSerial + lightSerial + '040100550a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '08bfaab9d8d7b4ccac08bfaab9d8d7b4ccac08bfaab9d8d7b4'
                          let cmd2 = '13aa710b468c020204ccac0000000000015' + (deviceSerial[7] - 1) + '3' (deviceConfig.type === 'color' ? '2' : '3') + '00'
                          // Brightness
                          let cmd3 = '3aaa7134448de0024131604f7448' + slotsRanges[deviceName][1].toString(16).padStart(2, '0') + panelSerial + lightSerial + '0e010055170a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '0ac1c1b6c8b0d9b7d6b1c8000000000000015' + (deviceSerial[7] - 1) + '0' (deviceConfig.type === 'color' ? '2' : '4') + '00'
                          // Name
                          let cmd4 = '37aa7131448ee202412e604f7448' + slotsRanges[deviceName][4].toString(16).padStart(2, '0') + panelSerial + lightSerial + '08001fa5140a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '08c9e8b1b8c3fbb3c60000000000015' + (deviceSerial[7] - 1) + '0' (deviceConfig.type === 'color' ? 'a' : 'b') + '00'
                          // Online/Offline
                          let cmd5 = '39aa7133448fdf024130604f7448' + slotsRanges[deviceName][5].toString(16).padStart(2, '0') + panelSerial + lightSerial + '080007fd160a0' + (deviceConfig.type === 'dim' ? '4' : '5') + '0ac9e8b1b8d4dacfdfc0eb0000000000015' + (deviceSerial[7] - 1) + '3' (deviceConfig.type === 'color' ? 'c' : 'd') + '00'
                          let commandsToExecute = [cmd1, cmd2, cmd3, cmd4, cmd5]
                          if (deviceConfig.type === 'ct') {
                            // Color Temperature
                            let cmd6 = '36aa713044670a02412d604f7448' + slotsRanges[deviceName][2].toString(16).padStart(2, '0') + panelSerial + lightSerial + '0e020055130a0506c9abcec2d6b5000000000000015' + (deviceSerial[7] - 1) + '0300'
                            commandsToExecute.push(cmd6)
                          } else if (deviceConfig.type === 'color') {
                            // Color
                            let cmd6 = '36aa713044234a02412d604f7448' + slotsRanges[deviceName][3].toString(16).padStart(2, '0') + panelSerial + lightSerial + '0e080055130a0506d1d5c9ab7879000000000000015' + (deviceSerial[7] - 1) + '0100'
                            commandsToExecute.push(cmd6)
                          }
                          for (var i = commandsToExecute.length - 1; i >= 0; i--) {
                            // separate commands with 500ms delay...
                            setTimeout(function() {
                              let cmdToSend = commandsToExecute[i];
                              this.put(resource, {aqara_s1_panel_communication: cmdToSend}).then((obj) => {
                        ...
                        if (deviceConfig && slotsRanges[deviceName]) {
                          ...
                          // TODO: Check that a light config haven't changed in a new config (Need to check how this is possible to be done... Maybe try to set CT/Color and see what is the response...)
                          let panelSerial = this.fullState.lights[r[2]].uniqueid.split('-')[0].replace(/:/g, '')
                          let acSerial = '6169725f636f6e64'
                          // On/Off, general type...
                          let cmd1 = '3aaa713444ef7e0241316044f76a' + slotsRanges[deviceName][0].toString(16).padStart(2, '0') + panelSerial + acSerial + (deviceConfig.internal_thermostat ? '0e020055' : '0e200055') + (deviceConfig.internal_thermostat ? '150a0608bfd8d6c6d7b4ccac000000000000012e0000' : '1708060abfd5b5f7d1b9cbf5d7b4000000000000012e0000')
                          // Online/Offline
                          let cmd2 = '39aa713344f07e0241306044f76a' + slotsRanges[deviceName][1].toString(16).padStart(2, '0') + panelSerial + acSerial + '080007fd' + '1608060ac9e8b1b8d4dacfdfc0eb0000000000012e6400'
                          // Name
                          let cmd3 = '37aa713144f17f02412e6044f76a' + slotsRanges[deviceName][2].toString(16).padStart(2, '0') + panelSerial + acSerial + '08001fa5' + '14080608c9e8b1b8c3fbb3c60000000000012e1300'
                          // Modes
                          let cmd4 = '39aa713344f27c0241306044f76a' + slotsRanges[deviceName][3].toString(16).padStart(2, '0') + panelSerial + acSerial + '08001fa7' + '1608060ab5b1c7b0c6a5c5e4b5c40000000000012e1000'
                          // Fan Modes
                          let cmd5 = '39aa713344f37b0241306044f76a' + slotsRanges[deviceName][4].toString(16).padStart(2, '0') + panelSerial + acSerial + '08001fa8' + '1608060ab5b1c7b0c6a5c5e4b5c40000000000012e1100'
                          // Temperatures Ranges
                          let cmd6 = '39aa713344f47a0241306044f76a' + slotsRanges[deviceName][5].toString(16).padStart(2, '0') + panelSerial + acSerial + '08001fa9' + '1608060ab5b1c7b0c6a5c5e4b5c40000000000012e0100'
