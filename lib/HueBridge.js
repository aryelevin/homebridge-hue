...
HueBridge.prototype.setAqaraS1PanelsConfiguration = function() {
        ...
        if (panelLightObject) {
          ...
          if (panelUniqueId[1] === '01') {
            ...
            for (var i = devicesSerial.length - 1; i >= 0; i--) {
              let deviceSerialIndex = i
              let deviceSerial = devicesSerial[deviceSerialIndex]
              let deviceName = devicesControl[deviceSerialIndex]
              ...
              if (slots) {
                if (deviceConfig) {
                  ...
                  } else if (deviceSerialIndex <= 7) { // curtains
                  ...
                  } else if (deviceSerialIndex == 8) { // AC
                    // On/Off, general type...
                  ...
                  if (!parsedData[deviceSerialIndex] || JSON.stringify(parsedData[deviceSerialIndex]) !== JSON.stringify(commandsToExecute)) {
                    ...
                    for (var ii = commandsToExecute.length - 1; ii >= 0; ii--) {
                      ...
                      let deviceIndex = deviceSerialIndex
                      ...
                      setTimeout(function() {
                        ...
                        that.put(panel + '/state', {aqara_s1_panel_communication: cmdToSend}).then((obj) => {
                          succeededCommands++
                          that.log.info('Sent: ' + cmdToSend + ', which is a command of index: ' + deviceIndex + ', succeeded: ' + succeededCommands + ', commands length: ' + commmands.length)
                    ...
                    // TODO: Maybe update the state of controlled device???
                  }
                } else {
  ...
  // TODO: update switches states (On/Off).
}
// End of Added by me: Arye Levin
      ...
      try {
        ...
        if (r[1] === 'sensors' && obj !== undefined && obj.buttonevent !== undefined && this.actionsConfigData && this.actionsConfigData[r[2]] && this.actionsConfigData[r[2]].length && this.state.remotesOn) {
          ...
          if (obj !== undefined && obj['aqara_s1_panel_communication'] !== undefined && obj['aqara_s1_panel_communication'] !== this.fullState.lights[r[2]].state.aqara_s1_panel_communication) {
                ...
                if (deviceResourceType) {
                  if (commandCategory === 0x72 && commandType === 0x84 && commandAction === 0x01 && this.state.remotesOn) { // State of device is reported.
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      ...
                      let resources = this.panelsToResources['/' + r[1] + '/' + r[2] + '/ac']
                      ...
                      let resources = this.panelsToResources['/' + r[1] + '/' + r[2] + '/curtain_' + deviceSerial[7]]
                      for (var i = resources.length - 1; i >= 0; i--) {
                        let resourceItem = resources[i];
                        let pathComponents = resourceItem.split( '/' )
                        let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
                    ...
                    if (deviceResourceType === 'air_cond' && stateParam[0] === 0x0e && stateParam[2] === 0x00 && stateParam[3] === 0x55 && (stateParam[1] === 0x20 || stateParam[1] === 0x02)) { // Updated Air conditioner/Heater-Cooler device state
                      let panelDevicePath = '/' + r[1] + '/' + r[2] + '/ac'
                      ...
                      let pathComponents = this.panelsToResources['/' + r[1] + '/' + r[2] + '/curtain_' + deviceSerial[7]][0].split( '/' )
                      let accessoryToControl = this[pathComponents[1]][pathComponents[2]]
            ...
            if (this.panelsToResources['/' + r[1] + '/' + r[2] + '/switch'] !== undefined && this.state.remotesOn) {
