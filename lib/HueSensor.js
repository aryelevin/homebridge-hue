...
HueSensor.prototype.updatePanels = function(hkValueToCheck) {
  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      if (panelResourcePathComponents[3] === 'ac') {
        var shouldUpdatePanelState = true
        if (hkValueToCheck) {
          let acsControlledWithPanelDevice = this.bridge.panelsToResources[panelResourceItem]
          for (var ii = acsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
            let acResourcePath = acsControlledWithPanelDevice[ii].split('/')
            let accessoryToCheck = this.bridge[acResourcePath[1]][acResourcePath[2]]
            if (accessoryToCheck) {
...
HueSensor.prototype.updatePanel = function(panelResourcePathComponents) {
  let internalThermostat = this.bridge.actionsConfigData.aqara_S1_panels['/' + panelResourcePathComponents[1] + '/' + panelResourcePathComponents[2]].ac.internal_thermostat
  let mode = this.service.getCharacteristic(Characteristic.TargetHeaterCoolerState).value
  let fan = this.service.getCharacteristic(Characteristic.RotationSpeed).value
  let temperature = mode < 2 ? this.service.getCharacteristic(mode === 0 ? Characteristic.HeatingThresholdTemperature : Characteristic.CoolingThresholdTemperature).value : 255

  let newState = (internalThermostat ? '0e020055' : '0e200055') + (this.service.getCharacteristic(Characteristic.Active).value === Characteristic.Active.ACTIVE ? '1' : '0') + (mode === Characteristic.TargetHeaterCoolerState.HEAT ? '0' : mode === Characteristic.TargetHeaterCoolerState.COOL ? '1' : '2') + (fan === 25 ? '0' : fan === 50 ? '1' : fan === 75 ? '2' : '3') + 'f' + temperature.toString(16).padStart(2, '0') + (internalThermostat ? Math.round((this.service.getCharacteristic(Characteristic.CurrentTemperature).value + 1) * 4).toString(16).padStart(2, '0') : '00')

  let panelResourcePath = '/' + panelResourcePathComponents[1] + '/' + panelResourcePathComponents[2] + '/state'
  this.bridge.put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + '6169725f636f6e64' + newState}).then((obj) => {
