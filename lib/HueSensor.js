    ...
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      if (pathComponents[3] === 'ac') {
        ...
        if (shouldUpdatePanelState) {
          ...
          let newState = '' + (this.service.getCharacteristic(Characteristic.Active).value ? '1' : '0') + (mode === Characteristic.TargetHeaterCoolerState.COOL ? '0' : mode === Characteristic.TargetHeaterCoolerState.HEAT ? '1' : '2') + (fan === 25 ? '1' : fan === 50 ? '2' : fan === 75 ? '3' : '0') + 'f' + (internalThermostat ? ((this.service.getCharacteristic(Characteristic.CurrentTemperature).value + 1) * 4).toString(16).padStart(2, '0') : '00')
          let panelResourcePath = '/' + pathComponents[1] + '/' + pathComponents[2] + '/state'
          this.bridge.put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + '6169725f636f6e64' + (internalThermostat ? '0e020055' : '0e200055') + newState}).then((obj) => {
