...
HueSensor.prototype.checkMode = function (mode) {
  ...
  if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
    ...
    let hkActive = 1//this.service.getCharacteristic(Characteristic.Active).value
    switch (mode) {
      case 'off':
        hkActive = 0
    ...
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      if (pathComponents[3] === 'ac') {
        ...
        if (shouldUpdatePanelState) {
          let internalThermostat = this.bridge.actionsConfigData.aqara_S1_panels['/' + pathComponents[1] + '/' + pathComponents[2]].ac.internal_thermostat
          ...
          let newState = (internalThermostat ? '0e020055' : '0e200055') + (this.service.getCharacteristic(Characteristic.Active).value ? '1' : '0') + (mode === Characteristic.TargetHeaterCoolerState.HEAT ? '0' : mode === Characteristic.TargetHeaterCoolerState.COOL ? '1' : '2') + (fan === 25 ? '1' : fan === 50 ? '2' : fan === 75 ? '3' : '0') + 'f' + temperature.toString(16).padStart(2, '0') + (internalThermostat ? Math.round((this.service.getCharacteristic(Characteristic.CurrentTemperature).value + 1) * 4).toString(16).padStart(2, '0') : '00')
