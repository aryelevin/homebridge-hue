...
function HueSensor (accessory, id, obj) {
  ...
  if (this.service) {
    ...
    if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
      ...
      try {
        let data = fs.readFileSync(__dirname + "/last_device_state_" + this.serialNumber, 'utf8');
...
HueSensor.prototype.setTargetHeaterCoolerState = function (
  ...
  let that = this;
  fs.writeFile(__dirname + "/last_device_state_" + this.serialNumber, JSON.stringify(this.hk), 'utf8', function(err) {
		if (err) {
			that.log.info(err);
		} else {
