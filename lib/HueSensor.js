...
function HueSensor (accessory, id, obj) {
  ...
  switch (this.obj.type) {
    ...
    case 'CLIPThermostat':
      // Added by me: Arye Levin
      if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
        this.service = new Service.HeaterCooler(this.name, this.subtype)
      } else {
      // End of Added by me: Arye Levin
      ...
      // Added by me: Arye Levin
      this.type.homekitValue = function (v) {
        this.updatePanels('temperature')
  ...
  if (this.service) {
    ...
    if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
      ...
      this.checkCoolSetPoint(this.obj.config.coolsetpoint)
      ...
      // Rotation Speed characteristic TODO: Think about allowing minValue of 0 since Home App just turn the AC off at this position in the UI...
      ...
      this.checkFanMode(this.obj.config.fanmode)
      this.service.addCharacteristic(Characteristic.SwingMode)
        .on('set', this.setSwingMode.bind(this))
      this.checkSwingMode(this.obj.config.swingmode)
      
      try {
        let data = fs.readFileSync(savedStateFilesPath + "last_device_state_" + this.serialNumber, 'utf8');
        let parsedJSON = JSON.parse(data);
        let hkTargetHeaterCoolerState = parsedJSON['targetHeaterCoolerState']
        this.hk.targetHeaterCoolerState = hkTargetHeaterCoolerState
        this.service.getCharacteristic(Characteristic.TargetHeaterCoolerState).updateValue(hkTargetHeaterCoolerState);
        this.log.info(this.name + ': The devices file was loaded!');
      } catch (err) {
...
// Added by me: Arye Levin
      case 'coolsetpoint':
        this.checkCoolSetPoint(config.coolsetpoint)
        break
      case 'fanmode':
        this.checkFanMode(config.fanmode)
        break
      case 'swingmode':
        this.checkSwingMode(config.swingmode)
        break
// End of Added by me: Arye Levin
      case 'lastchange':
        break
...
// Added by me: Arye Levin
HueSensor.prototype.checkCoolSetPoint = function (coolsetpoint) {
  if (this.obj.config.coolsetpoint !== coolsetpoint) {
    this.log.debug(
      '%s: sensor coolsetpoint changed from %j to %j', this.name,
      this.obj.config.coolsetpoint, coolsetpoint
    )
    this.obj.config.coolsetpoint = coolsetpoint
  }
  const hkCoolTemperature = Math.round(this.obj.config.coolsetpoint / 50) / 2
  if (this.hk.coolTemperature !== hkCoolTemperature) {
    if (this.hk.coolTemperature !== undefined) {
      this.log.info(
        '%s: set homekit cool temperature from %s°C to %s°C', this.name,
        this.hk.coolTemperature, hkCoolTemperature
      )
    }
    this.hk.coolTemperature = hkCoolTemperature
    this.service.updateCharacteristic(
      Characteristic.CoolingThresholdTemperature, this.hk.coolTemperature
    )
  }
}

HueSensor.prototype.checkFanMode = function (fanmode) {
  if (this.obj.config.fanmode !== fanmode) {
    this.log.debug(
      '%s: sensor fanmode changed from %j to %j', this.name,
      this.obj.config.fanmode, fanmode
    )
    this.obj.config.fanmode = fanmode
  }
  const fanState = {auto:100, low:25, medium:50, high:75};
  const hkRotationSpeed = fanState[this.obj.config.fanmode]
  if (this.hk.rotationSpeed !== hkRotationSpeed) {
    if (this.hk.rotationSpeed !== undefined) {
      this.log.info(
        '%s: set homekit rotation speed from %s to %s', this.name,
        this.hk.rotationSpeed, hkRotationSpeed
      )
    }
    this.hk.rotationSpeed = hkRotationSpeed
    this.service.updateCharacteristic(
      Characteristic.RotationSpeed, this.hk.rotationSpeed
    )
  }
}

HueSensor.prototype.checkSwingMode = function (swingmode) {
  if (this.obj.config.swingmode !== swingmode) {
    this.log.debug(
      '%s: sensor swingmode changed from %j to %j', this.name,
      this.obj.config.swingmode, swingmode
    )
    this.obj.config.swingmode = swingmode
  }
  const swingStates = {'fully open':0, 'half open':1, 'quarter open':1, 'three quarters open':1, 'fully closed':1};
  const hkSwingMode = swingStates[this.obj.config.swingmode]
  if (this.hk.swingMode !== hkSwingMode) {
    if (this.hk.swingMode !== undefined) {
      this.log.info(
        '%s: set homekit swing mode from %s to %s', this.name,
        this.hk.swingMode, hkSwingMode
      )
    }
    this.hk.swingMode = hkSwingMode
    this.service.updateCharacteristic(
      Characteristic.SwingMode, this.hk.swingMode
    )
  }
}
// End of Added by me: Arye Levin
...
HueSensor.prototype.checkHeatSetPoint = function (heatsetpoint) {
  ...
  if (this.hk.targetTemperature !== hkTargetTemperature) {
    ...
    if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
    this.service.updateCharacteristic(
      Characteristic.HeatingThresholdTemperature, this.hk.targetTemperature
    )
    } else 
    // End of Added by me: Arye Levin
    this.service.updateCharacteristic(
...
HueSensor.prototype.checkMode = function (mode) {
  ...
  if (this.obj.config.coolsetpoint !== undefined && this.obj.config.fanmode !== undefined) {
    ...
    let hkTargetHeaterCoolerState = this.service.getCharacteristic(Characteristic.TargetHeaterCoolerState).value
    let hkCurrentHeaterCoolerState = this.service.getCharacteristic(Characteristic.CurrentHeaterCoolerState).value
    let hkActive = Characteristic.Active.ACTIVE//this.service.getCharacteristic(Characteristic.Active).value
    switch (mode) {
      case 'off':
        hkActive = Characteristic.Active.INACTIVE
        hkCurrentHeaterCoolerState = Characteristic.CurrentHeaterCoolerState.INACTIVE
        break
      case 'cool':
        hkTargetHeaterCoolerState = Characteristic.TargetHeaterCoolerState.COOL
        hkCurrentHeaterCoolerState = Characteristic.CurrentHeaterCoolerState.COOLING
        break
      case 'heat':
        hkTargetHeaterCoolerState = Characteristic.TargetHeaterCoolerState.HEAT
        hkCurrentHeaterCoolerState = Characteristic.CurrentHeaterCoolerState.HEATING
        break
      case 'auto':
      default:
        hkTargetHeaterCoolerState = Characteristic.TargetHeaterCoolerState.AUTO
        hkCurrentHeaterCoolerState = Characteristic.CurrentHeaterCoolerState.IDLE
        break
    }
    if (this.hk.targetHeaterCoolerState !== hkTargetHeaterCoolerState) {
      if (this.hk.targetHeaterCoolerState !== undefined) {
        this.log.info(
          '%s: set homekit target heater cooler state from %s to %s', this.name,
          this.hk.targetHeaterCoolerState, hkTargetHeaterCoolerState
        )
      }
      this.hk.targetHeaterCoolerState = hkTargetHeaterCoolerState
      this.service.getCharacteristic(Characteristic.TargetHeaterCoolerState)
        .updateValue(this.hk.targetHeaterCoolerState)
    }
    if (this.hk.currentHeaterCoolerState !== hkCurrentHeaterCoolerState) {
      if (this.hk.currentHeaterCoolerState !== undefined) {
        this.log.info(
          '%s: set homekit current heater cooler state from %s to %s', this.name,
          this.hk.currentHeaterCoolerState, hkCurrentHeaterCoolerState
        )
      }
      this.hk.currentHeaterCoolerState = hkCurrentHeaterCoolerState
      this.service.getCharacteristic(Characteristic.CurrentHeaterCoolerState)
        .updateValue(this.hk.currentHeaterCoolerState)
    }
    if (this.hk.active !== hkActive) {
      if (this.hk.active !== undefined) {
        this.log.info(
          '%s: set homekit active state from %s to %s', this.name,
...
// Added by me: Arye Levin
HueSensor.prototype.setTargetHeaterCoolerState = function (
  targetHeaterCoolerState, callback
) {
  if (targetHeaterCoolerState === this.hk.targetHeaterCoolerState) {
    return callback()
  }
  this.log.info(
    '%s: homekit target heater cooler state changed from %s to %s', this.name,
    this.hk.targetHeaterCoolerState, targetHeaterCoolerState
  )
  this.hk.targetHeaterCoolerState = targetHeaterCoolerState
  let mode
  switch (this.hk.targetHeaterCoolerState) {
  ...
  let that = this;
  fs.writeFile(savedStateFilesPath + "last_device_state_" + this.serialNumber, JSON.stringify(this.hk), 'utf8', function(err) {
		if (err) {
			that.log.info(err);
		} else {
...
HueSensor.prototype.setCoolingThresholdTemperature = function (coolingThresholdTemperature, callback) {
  if (coolingThresholdTemperature === this.hk.coolTemperature) {
    return callback()
  }
  this.log.info(
    '%s: homekit target temperature changed from %s to %s', this.name,
    this.hk.coolTemperature, coolingThresholdTemperature
  )
  this.hk.coolTemperature = coolingThresholdTemperature
  const hueCoolSetPoint = Math.round(coolingThresholdTemperature * 100)
  this.put('/config', { coolsetpoint: hueCoolSetPoint }).then((obj) => {
    this.obj.config.coolsetpoint = hueCoolSetPoint
    return callback()
  }).catch((error) => {
    return callback(error)
  })

  this.updatePanels('coolTemperature')
}

HueSensor.prototype.setHeatingThresholdTemperature = function (heatingThresholdTemperature, callback) {
  if (heatingThresholdTemperature === this.hk.targetTemperature) {
    return callback()
  }
  this.log.info(
    '%s: homekit target temperature changed from %s to %s', this.name,
    this.hk.targetTemperature, heatingThresholdTemperature
  )
  this.hk.targetTemperature = heatingThresholdTemperature
  const hueHeatSetPoint = Math.round(heatingThresholdTemperature * 100)
  ...
  this.updatePanels('targetTemperature')
}
...
HueSensor.prototype.setActive = function (active, callback) {
  ...
  this.log.info(
    '%s: homekit active changed from %s to %s', this.name,
...
HueSensor.prototype.setRotationSpeed = function (rotationSpeed, callback) {
  if (rotationSpeed === this.hk.rotationSpeed) {
    return callback()
  }
  this.log.info(
    '%s: homekit rotation speed changed from %s to %s', this.name,
    this.hk.rotationSpeed, rotationSpeed
  )
  this.hk.rotationSpeed = rotationSpeed
  let speed = 'auto'
  switch (this.hk.rotationSpeed) {
    case 25:
      speed = 'low'
      break
    case 50:
  ...
  this.put('/config', { fanmode: speed }).then((obj) => {
    this.obj.config.fanmode = speed
    return callback()
  }).catch((error) => {
    return callback(error)
  })

  this.updatePanels('rotationSpeed')
}

HueSensor.prototype.setSwingMode = function (swingMode, callback) {
  if (swingMode === this.hk.swingMode) {
    return callback()
  }
  this.log.info(
    '%s: homekit swing mode changed from %s to %s', this.name,
    this.hk.swingMode, swingMode
  )
  this.hk.swingMode = swingMode
  let mode = this.hk.swingMode > 0 ? 'half open' : 'fully open'
  
  this.put('/config', { swingmode: mode }).then((obj) => {
    this.obj.config.swingmode = mode
    return callback()
  }).catch((error) => {
    return callback(error)
  })
...
HueSensor.prototype.updatePanels = function(hkValueToCheck) {
  ...
  if (panelsToUpdate && Array.isArray(panelsToUpdate) && panelsToUpdate.length) {
    for (var i = panelsToUpdate.length - 1; i >= 0; i--) {
      ...
      if (panelResourcePathComponents[4] === 'ac') {
        ...
        if (hkValueToCheck) {
          let acsControlledWithPanelDevice = this.bridge.platform.panelsToResources[panelResourceItem]
          for (var ii = acsControlledWithPanelDevice.length - 1; ii >= 0; ii--) {
            let acResourcePath = acsControlledWithPanelDevice[ii].split('/')
            let accessoryToCheck = this.bridge.platform.bridgeMap[acResourcePath[1]][acResourcePath[2]][acResourcePath[3]]
            if (accessoryToCheck) {
              if (accessoryToCheck !== this && accessoryToCheck.hk[hkValueToCheck] !== undefined && accessoryToCheck.hk[hkValueToCheck] != this.hk[hkValueToCheck]) {
...
HueSensor.prototype.updatePanel = function(panelResourcePathComponents) {
  let internalThermostat = this.bridge.platform.configJson.actionsConfigData[panelResourcePathComponents[1]].aqara_S1_panels['/' + panelResourcePathComponents[2] + '/' + panelResourcePathComponents[3]].ac.internal_thermostat
  let temperature = this.hk.targetHeaterCoolerState === Characteristic.TargetHeaterCoolerState.HEAT ? this.hk.targetTemperature : this.hk.targetHeaterCoolerState === Characteristic.TargetHeaterCoolerState.COOL ? this.hk.coolTemperature : 255

  let newState = (internalThermostat ? '0e020055' : '0e200055') + (this.hk.active === Characteristic.Active.ACTIVE ? '1' : '0') + (this.hk.targetHeaterCoolerState === Characteristic.TargetHeaterCoolerState.HEAT ? '0' : this.hk.targetHeaterCoolerState === Characteristic.TargetHeaterCoolerState.COOL ? '1' : '2') + (this.hk.rotationSpeed === 25 ? '0' : this.hk.rotationSpeed === 50 ? '1' : this.hk.rotationSpeed === 75 ? '2' : '3') + (internalThermostat ? '0' : 'f') + temperature.toString(16).padStart(2, '0') + (internalThermostat ? ((Math.round(this.hk.temperature) + 0) * 4).toString(16).padStart(2, '0') : '00')

  let panelResourcePath = '/' + panelResourcePathComponents[2] + '/' + panelResourcePathComponents[3] + '/state'
  this.bridge.platform.bridgeMap[panelResourcePathComponents[1]].put(panelResourcePath, {aqara_s1_panel_communication: '19aa7113446d21054110' + '6169725f636f6e64' + newState}).then((obj) => {
